...
const AdminPanel = ({ currentEvents, onAddEvents, onTogglePublish }) => {
  ...
  const fetchCalendarEvents = async () => {
    ...
    try {
      ...
      // ★修正箇所：色がない場合（既定）も含めてリストアップする
      const newCandidates = (data.items || [])
        .filter(event => {
           // 色指定がある場合は対象のみ、ない場合(既定)も許可する
           if (event.colorId) {
             return targetColorIds.includes(event.colorId);
           }
           return true; // 既定の色（colorIdなし）も表示する
        })
        .map(event => {
          ...
          return {
            ...
            colorId: event.colorId || 'default' // 色がない場合はdefaultとする
          };
        });

      setFetchedEvents(newCandidates);
      
      // ★修正箇所：自動選択するのは、明示的に色が合致するものだけにする
      const autoSelectIds = newCandidates
        .filter(e => e.colorId !== 'default')
        .map(e => e.id);
      ...
      <div className="flex flex-col items-center justify-center py-12 px-4">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 w-full max-w-sm text-center">
          ...
          <form onSubmit={handleLogin} className="mt-4">
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3 mb-4 focus:ring-2 focus:ring-indigo-500 outline-none transition-all" placeholder="パスワード" />
            <button type="submit" className="w-full bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-800 active:scale-[0.98] transition-all">認証する</button>
          </form>
        </div>
      </div>
    );
